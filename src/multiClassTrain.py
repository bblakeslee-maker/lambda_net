...
def main():
    ...
    trainData = loader.Dataset(config, 'train',
    ...
    # Configure loss
    addChangeWeight = config['params'].getfloat('addChangeWeight')
    subChangeWeight = config['params'].getfloat('subChangeWeight')
    exchangeWeight = config['params'].getfloat('exchangeWeight')
    weightVector = torch.tensor([1, addChangeWeight, subChangeWeight,
                                exchangeWeight], dtype=torch.float)
    weightVector = weightVector.to(device)
    lossCriteria = nn.CrossEntropyLoss(weight=weightVector)
    print('STATUS: Running weighted CrossEntropyLoss with '
          'weights {}!'.format(weightVector))
